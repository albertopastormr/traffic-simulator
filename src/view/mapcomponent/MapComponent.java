...
public class MapComponent extends JComponent implements ObserverTrafficSimulator {
	...
	private static final int radioVehicle = 5;
	...
	private void drawMap(Graphics g) {
		...
		for (Road e : this.mapa.getRoads()) {
			Punto p1 = posicionesDeNodos.get(e.getOrigin().getId());
			Punto p2 = posicionesDeNodos.get(e.getDestination().getId());
			...
			int diam = radioVehicle;
			for (Vehicle d : e.getVehicles()) {
				if (ultimaLocalizacion != d.getLocationActual()) {
					ultimaLocalizacion = d.getLocationActual();
					diam = radioVehicle;
				} else {
					diam += radioVehicle;
				}
				Color dotColor = d.getBreakdownTime() > 0.5 ? Color.RED : Color.MAGENTA;
				drawCircleOnALine(g, p1.cX, p1.cY, p2.cX, p2.cY, e.getLength(), d.getLocationActual(), diam, dotColor,
						d.getId());
			}
		}
	}
	...
	private boolean hasGreen(Road e) {
		for (InRoad r : e.getDestination().getInRoads()) {
	...
	@Override
	public void simulatorError(int tiempo, RoadMap mapa, List<Event> eventos, SimulationError e) {
